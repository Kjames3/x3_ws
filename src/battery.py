...
# Longest gap between samples that is integrated at face value, seconds.
#
# The caller samples at 1 Hz, so anything past a minute means the clock moved
# without the pack moving with it.  On this Jetson that is not hypothetical:
# there is no battery-backed RTC, so x3_server starts at epoch ~60 and NTP then
# steps the clock forward ~1.79e9 s a few seconds later.  Integrating that one
# dt at a 0.9 A idle draw subtracts ~447,000 Ah, which pins the counter at 0 and
# leaves the gauge reading 0% on a full pack -- exactly what happened on
# 2026-08-20 (``logs/battery/battery_19691231-160102.csv``, row 1 -> row 2:
# 100.00% -> 0.00% across a single sample).
#
# The call site now passes a monotonic clock, which is the actual fix; this
# clamp is the backstop, because a frozen event loop, a suspend, or any future
# caller that reaches for time.time() again would otherwise destroy the
# integral in one sample.  Clamping rather than skipping keeps a genuine stall
# accounted for: the pack really was drawing during it, so the last known
# current is the best available estimate for the first minute of it, and
# under-counting a long gap is the safe direction for a fuel gauge.
MAX_SAMPLE_DT_S = 60.0
...
class BatteryEstimator:
    ...
    def update(self, volts: float, current_a: float = 0.0,
        ...
        ``now`` **must come from a monotonic clock**; it defaults to
        :func:`time.monotonic` for that reason.  A wall clock is not merely
        imprecise here, it is unsound: this machine steps ``time.time()``
        forward by 56 years shortly after boot, and the gap between two samples
        is multiplied straight into the charge integral.  Gaps longer than
        :data:`MAX_SAMPLE_DT_S` are clamped so that a clock step or a stalled
        loop can no longer empty the counter in one sample.
        """
        ...
        if self._v_ema is None or self._last_t is None:
            ...
            # Clamped, not merely floored at zero: a backward step costs one
            # sample, but an unclamped forward step would be integrated in full.
            dt = min(MAX_SAMPLE_DT_S, max(0.0, now - self._last_t))
