...
# --- clock robustness ------------------------------------------------------
# Regression cover for the 2026-08-20 failure: the GUI read 0% on a pack
# sitting at 13.11 V.  The Jetson has no battery-backed RTC, so x3_server
# starts at epoch ~60 and NTP steps the wall clock forward ~1.79e9 s seconds
# later.  battery_loop() was passing time.time() into update(), so that step
# arrived as a single 56-year dt and the counter was drained on the second
# sample of every boot.
def test_a_clock_step_cannot_drain_the_counter():
    est = BatteryEstimator(initial_charge_ah=PACK_CAPACITY_AH * 0.78)
    est.update(13.11, 0.9, 62.0)
    before = est.percent
    est.update(13.11, 0.9, 62.0 + 1.787e9)     # NTP steps the clock
    assert est.percent > before - 1.0, \
        f"a clock step moved the gauge {before:.1f}% -> {est.percent:.1f}%"


def test_a_clock_step_cannot_zero_a_freshly_seeded_pack():
    """End-to-end shape of the observed failure: 100% -> 0% in one sample."""
    est = BatteryEstimator()
    est.update(13.14, 0.85, 62.0)              # cold seed from OCV, near full
    assert est.percent > 70.0, \
        f"fixture assumption broken: cold seed read {est.percent:.1f}%"
    est.update(13.11, 0.9, 62.0 + 1.787e9)
    assert est.percent > 70.0, \
        f"pack at 13.11 V reported {est.percent:.1f}%"


def test_a_long_gap_is_still_charged_for_but_bounded():
    """Clamped, not ignored: a real stall did draw current, just not 56 years'."""
    est = BatteryEstimator(initial_charge_ah=PACK_CAPACITY_AH)
    est.update(12.99, 3.6, 0.0)
    est.update(12.99, 3.6, 3600.0)             # loop stalled for an hour
    drawn = PACK_CAPACITY_AH - est.charge_ah
    expected = 3.6 * battery.MAX_SAMPLE_DT_S / 3600.0
    assert 0.0 < drawn <= expected * 1.05, \
        f"expected at most ~{expected:.3f} Ah for a clamped gap, got {drawn:.3f}"


def test_backward_clock_step_costs_at_most_one_sample():
    est = BatteryEstimator(initial_charge_ah=PACK_CAPACITY_AH)
    est.update(12.99, 2.0, 1.787e9)
    est.update(12.99, 2.0, 62.0)               # clock jumps backwards
    assert est.charge_ah == PACK_CAPACITY_AH, "a backward step must not add charge"
    _soak(est, 12.99, 2.0, 3600, t0=62.0, step=10.0)
    assert est.charge_ah < PACK_CAPACITY_AH - 1.5, \
        f"counter stayed frozen after a backward step: {est.charge_ah:.3f} Ah"


def test_normal_sampling_is_untouched_by_the_clamp():
    """The clamp must not quietly change the 1 Hz path the robot actually uses."""
    saved = battery.OCV_VOLTAGE_UNCERTAINTY_V
    battery.OCV_VOLTAGE_UNCERTAINTY_V = 1e9
    try:
        est = BatteryEstimator(initial_charge_ah=PACK_CAPACITY_AH)
        _soak(est, 12.98, 2.0, 3600, step=1.0)
        used = PACK_CAPACITY_AH - est.charge_ah
    finally:
        battery.OCV_VOLTAGE_UNCERTAINTY_V = saved
    assert 1.95 < used < 2.05, f"expected ~2 Ah drawn, got {used:.3f}"
